<template>
  <div class="login-wrap">
    <div style="text-align: center; margin-bottom: 30px; font-size: 30px">旅游信息分享管理系统</div>
    ...
    <!--<el-row :gutter="20" class="footer">-->
       <!--<el-col :span="12" :offset="11"><div><span>@ZJH 2022</span></div></el-col>-->
    <!--</el-row>-->
  </div>
</template>
        ...
        if (valid) {
          ...
          login(this.ruleForm).then(res => {
            ...
            if (res.success) {
              ...
              setTimeout(() => {
                this.logining = false
                localStorage.setItem('menu',  res.data.row.role);
                // 缓存token
                localStorage.setItem('account', res.data.row.id);
                // 缓存用户个人信息
                localStorage.setItem('userdata', JSON.stringify(res.data.row))
                this.$store.commit('login', 'true')
                this.$router.push({ path: '/route/route' })
              }, 1000)
            } else {
